...
class Coordinate {
    ...
    static NUMPLAYERS = new ClosedInterval(0,2);
...
class TurnView {
    ...
    config(){
        ...
        let error = false;
        do {
            response = console.readNumber(`Modo de juego: [0] Machine,  [1] Machine-Human, [2] Human-Human`)
            error = !Coordinate.NUMPLAYERS.isIncluded(response)
            if (error){
                console.writeln(`Error ${response} no es válido`)
            }
        } while (error);
    }
